...
<script type="text/javascript">
	...
	function addReply() { //댓글 등록 요청
		...
		/*   alert(params); */
		new ajax.xhr.Request("/Dobid/noticeboard_content_view_reply_add.do", params, addResult, 'POST');
	...
	function updateReply(no, name) { //댓글 수정 요청
		...
		new ajax.xhr.Request("/Dobid/noticeboard_content_view_reply_update.do", params, updateResult, 'POST');
	...
	function loadReplyList() { //목록요청
		new ajax.xhr.Request('/Dobid/noticeboard_content_viewreply_list.do', "num=${param.num}", loadReplyResult);
	} //loadReplyList
...
<body>
	<div style="width: 40%; margin-left: 33.3%">
		...
		<!-- 댓글 입력폼 -->
		<div id="replyAdd"
			style="margin-top: 30px; border-top: 1px solid black;">
			<form name="addForm">
				...
				<div style="width: 100%; margin-top: 10px;">
					...
					<input type="hidden" name="num" size="10"
						value="${noticeobject.basic_board_num }" style="display:;"
						readonly="readonly"> <input type="hidden" name="nickname"
						size="10" value="${nickname }" style="display:;"
						readonly="readonly">
				</div>
				<div>
					<textarea rows="5" name="content"
						style="vertical-align: middle; width: 75%"></textarea>
					<input type="button" value="등록"
						style="width: 18%; height: 70px; margin-left: 3%"
						onclick="addReply()">
				</div>
			</form>
		</div>
		...
		<div id="replyUpdate" style="display: none; margin: 10px;">
			<form name="updateForm">
				<input type="hidden" name="no"> <input type="hidden"
					name="nickname" size="10" style="display:;" readonly="readonly">
				<input type="hidden" name="num" size="5"
					value="${noticeobject.basic_board_num }" style="display:;"
					readonly="readonly"> <input type="hidden" name="name"
					size="10" readonly="readonly"><br>

				<textarea rows="5" style="width: 105%" name="content"></textarea>
				<br>
				<br> <input type="button" value="취소" style="float: right;"
					onclick="hideUpdateForm()"> <input type="button" value="등록"
					style="float: right;" onclick="updateReply()"> <br>
			</form>
		</div>
	</div>
