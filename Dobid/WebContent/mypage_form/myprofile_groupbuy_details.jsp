...
<body>
	...
	<br>
		...
		<font size="80" color="black">공동구매내역</font><br>
		</div>
	...
	<div class="container" style="size: auto;width: 55%;">
      ...
      <c:if test="${page > 1}">
         <a href="myprofile_groupbuylist.do?page=${ page-1 }">이전</a>      
      </c:if>
         
      <c:if test="${page == totalPage }">다음</c:if>
      <c:if test="${page < totalPage }">
         <a href="myprofile_groupbuylist.do?page=${ page+1 }">다음</a>      
      </c:if>
    <br><br>
    <c:if test="${totalPage>0}">
    <c:forEach begin="1" end="${totalPage }" var="i">
       [<a href="myprofile_groupbuylist.do?page=${ i }">${i }</a>]
    </c:forEach>
    </c:if>
