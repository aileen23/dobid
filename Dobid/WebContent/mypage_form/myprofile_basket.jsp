...
<body>
	...
	<div class="container">
      ...
      <c:if test="${page > 1}">
         <a href="/struts/guest2/control.do?action=list&page=${ page-1 }">이전</a>      
      </c:if>
         
      <c:if test="${page == totalPage }">다음</c:if>
      <c:if test="${page < totalPage }">
         <a href="/struts/guest2/control.do?action=list&page=${ page+1 }">다음</a>      
      </c:if>
    <br><br>
    <c:if test="${totalPage>1}">
    <c:forEach begin="1" end="${totalPage }" var="i">
       [<a href="/struts/guest2/control.do?action=list&page=${ i }">${i }</a>]
    </c:forEach>
    </c:if>
